...
import { LogOut, LayoutDashboard, CheckSquare, Users, FileBarChart, Bell, AlertTriangle, Clock, X, ChevronRight, Database, ShieldCheck } from 'lucide-react';
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex-shrink-0 fixed h-full z-30 hidden md:flex md:flex-col shadow-sm">
        ...
        <div className="px-4 py-3 bg-slate-50 border-t border-b border-slate-100">
            <div className="flex items-center gap-2 text-emerald-600">
                <ShieldCheck size={14} />
                <span className="text-[10px] font-bold uppercase tracking-wider">Storage Criptografado</span>
            </div>
            <div className="flex items-center gap-2 text-blue-600 mt-1">
                <Database size={14} />
                <span className="text-[10px] font-bold uppercase tracking-wider">IndexedDB Online</span>
            </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center justify-between mb-4 px-2">
            ...
            <div className="relative" ref={notificationRef}>
              <button onClick={handleNotificationClick} className="text-slate-500 hover:text-blue-600 transition-colors relative p-1">
                <Bell size={20} />
                {hasAlerts && <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white animate-pulse"></span>}
              </button>
              {showNotifications && (
                <div className="absolute bottom-10 left-full ml-2 w-80 bg-white rounded-lg shadow-xl border border-slate-200 z-50 overflow-hidden animate-fade-in">
                   ...
                   <div className="max-h-80 overflow-y-auto">
                      {alerts.length === 0 ? <div className="p-6 text-center text-slate-400 text-sm">Nenhuma notificação.</div> : alerts.map((alert, idx) => (
                          <div key={idx} onClick={() => handleAlertItemClick(alert)} className={`p-3 border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors flex items-start gap-3 ${alert.type === 'overdue' ? 'bg-red-50/30' : 'bg-orange-50/30'}`}>
                              <div className={`mt-0.5 shrink-0 ${alert.type === 'overdue' ? 'text-red-500' : 'text-orange-500'}`}>{alert.type === 'overdue' ? <AlertTriangle size={16} /> : <Clock size={16} />}</div>
                              <div className="flex-1 min-w-0">
                                  <p className={`text-xs font-bold mb-0.5 ${alert.type === 'overdue' ? 'text-red-700' : 'text-orange-700'}`}>{alert.type === 'overdue' ? 'Tarefa Atrasada' : 'Prazo Próximo'}</p>
          ...
          <button onClick={onLogout} className="w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm text-red-600 bg-red-50 hover:bg-red-100 rounded-md transition-colors">
            <LogOut size={16} />
            <span>Sair</span>
          </button>
        </div>
      ...
      <div className="md:hidden fixed top-0 w-full bg-white border-b border-slate-200 z-30 px-4 py-3 flex justify-between items-center shadow-sm h-14">
        ...
        <div className="flex items-center gap-4">
            <div className="relative">
                <button onClick={handleNotificationClick} className="text-slate-500 relative p-1">
                  <Bell size={22} />
                  {hasAlerts && <span className="absolute top-0 right-0 flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span></span>}
                </button>
            </div>
            <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 font-medium bg-slate-100 px-2 py-1 rounded-full">{user.role === UserRole.MANAGER ? 'Gerente' : user.role === UserRole.ADMIN ? 'Admin' : `${user.shift}`}</span>
                <button onClick={onLogout} className="text-red-600 p-1"><LogOut size={20}/></button>
            </div>
        </div>
      </div>
